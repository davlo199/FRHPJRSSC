...
library("PtProcess")
  ...
  dataname<-gsub(" ", "", paste(data,".csv"))
  XX<-read.csv(dataname,head=T)
  XX<-XX[XX$mag>=M0,]
  #colnames(XX)[c(2,3)]<-c("lat","long")
  #x$Date=as.Date(x$time)
  #x$time<-paste(x$Hr,":",x$Min,":",x$Sec,sep="") #overwrites time so that it is GMT
  xEtas2=c()
  ...
  xEtas2$time<-as.numeric(difftime(XX$time,XX$time[1],units="days"))
  xEtas2$magnitude<-(XX$mag-min(XX$mag))##Relative magbitude
  xEtas2<-as.data.frame(xEtas2)
  xEtas2<-xEtas2[A:B,]
